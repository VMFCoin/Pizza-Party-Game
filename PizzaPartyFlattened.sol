...
// foundry/src/PizzaPartyDeploy.sol
...
contract PizzaParty is Ownable, ReentrancyGuard {
    ...
    function _settleWeeklyGame(uint256 weekId) internal {
        ...
        // BUGFIX: Use actual winners count (may be less than requested if selection couldn't find enough unique winners)
        uint256 actualWinnerCount = winners.length;

        // Edge case: no winners selected (shouldn't happen but handle gracefully)
     ...
     * @param referralCodes Array of referral codes for each player (empty string if none)
     */
    function migratePlayerStats(
        ...
        for (uint256 i = 0; i < players.length; i++) {
            ...
            // Migrate referral code if provided
            if (bytes(referralCodes[i]).length > 0) {
                string memory code = referralCodes[i];
                // Only set if not already registered (avoid overwriting)
