...
async function checkAllStats() {
  try {
    ...
    console.log("Checking all player stats...\n");
    ...
    for (let i = 0; i < playerAddresses.length; i++) {
      ...
      try {
        ...
        if (hasStats) {
          ...
          results.push({ address: player, hasStats: true });
        } else {
          console.log(`${i + 1}. ${player} - ✗ No stats`);
          results.push({ address: player, hasStats: false });
        }
      } catch (error) {
        console.log(`${i + 1}. ${player} - ✗ Error: ${error.message}`);
        results.push({ address: player, hasStats: false, error: true });
      }
    ...
    console.log(`  • Players without stats: ${playerAddresses.length - playersWithStats}`);

    if (playersWithStats < 22) {
      console.log(`\n⚠️  Only ${playersWithStats} of 22 expected players have stats.`);
