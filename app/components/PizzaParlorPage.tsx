...
export default function PizzaParlorPage({
  ...
  const handleSendSlice = async () => {
    ...
    // Check if this specific sender→recipient pair is blocked
    if (isSlicePairBlocked(userFid, recipientFid)) {
      setSelfSliceError("Sending slices to this player has been blocked.")
      return
    }
          ...
          <div className="space-y-3">
                    ...
                    >
                      {isDistributing || (isConfirming && isDistributing)
                        ...
                        : claimableFeesFormatted <= 0
                          ? '💰 NO FEES TO COLLECT 💰'
                          : `💰 COLLECT ${claimableFeesFormatted.toLocaleString()} PIZZA 💰`}
                    </Button>
                  </div>
                </div>
              )}
            ...
            <Card className="border-4 border-orange-600 rounded-2xl bg-white/95 !py-0">
              <div className="px-3 py-2">
                ...
                <ul className="space-y-0.5 text-orange-800 text-sm" style={{ fontFamily: 'var(--font-luckiest-guy)' }}>
