...
/**
 * @title PizzaPartyV2
 * @dev Daily lottery + Weekly jackpot with topping-based tickets using $PIZZA token
 *
 ...
 * - Holdings: 3 toppings per 1k PIZZA held (snapshot at claim time)
 * - Slice entries: 1 topping (from parlor owners)
 */
contract PizzaPartyV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public ownerFeeBPS = 300; // Default 3%, adjustable up to MAX_OWNER_FEE_BPS
    ...
    function _settleWeeklyGame(uint256 weekId) internal {
        ...
        // Jackpot = total claimed toppings × 10 PIZZA
